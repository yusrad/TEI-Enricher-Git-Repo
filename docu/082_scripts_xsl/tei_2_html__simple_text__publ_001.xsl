<xsl:stylesheet xml:space="preserve" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" version="2.0" xmlns:egXML="http://www.tei-c.org/ns/Examples" exclude-result-prefixes="xsl tei egXML">
   ...
   <xsl:template match="tei:ab[@type='caption']">
      <div class="abCaption"><xsl:apply-templates/></div>
   </xsl:template>

   <xsl:template match="tei:cell"><td><xsl:attribute name="class"><xsl:value-of select="@role"/></xsl:attribute><xsl:attribute name="style"><xsl:value-of select="@style"/></xsl:attribute>
   ...
   <xsl:template match="tei:figure">
      <img src="{tei:graphic/@url}" alt="{tei:graphic/@url}"/><xsl:apply-templates/>
   </xsl:template>
   <xsl:template match="tei:head">
      <xsl:variable name="depth" select="count(ancestor::tei:div)"/>
      <xsl:variable name="sid" select="parent::tei:div/@xml:id"/>
      <xsl:choose>
         <xsl:when test="$depth=0"><h1 class="head1_">
<!--
               <xsl:variable name="num1"><xsl:value-of select="count(parent::tei:div/preceding-sibling::tei:div)+1"/></xsl:variable>
               <xsl:value-of select="$num1"/><xsl:text>
               </xsl:text>
               -->
               <xsl:apply-templates/></h1></xsl:when>
...
-->
               <xsl:apply-templates/></h2></xsl:when>
...
<!--
               ...
               -->
               <xsl:apply-templates/></h3></xsl:when>
...
<!--
               ...
               <xsl:value-of select="concat($num1, '.', $num2)"/><xsl:text>
               </xsl:text>
               -->
               <xsl:apply-templates/></h4></xsl:when>
...
<!--
               ...
               <xsl:value-of select="concat($num1, '.', $num2, '.', $num3)"/><xsl:text>
               </xsl:text>
               -->
               <xsl:apply-templates/></h5></xsl:when>
...
<!--
               ...
               <xsl:value-of select="concat($num1, '.', $num2, '.', $num3, '.', $num4)"/><xsl:text>
               </xsl:text>
               -->
   ...
   <xsl:template match="tei:p[tei:figure]"><div><xsl:apply-templates/></div></xsl:template>
   ...
   <xsl:template match="tei:name[@type='application' or @type='appPart']">
      <span class="spApp"><xsl:apply-templates/></span>
   </xsl:template>

   <xsl:template match="tei:name[@type='filename']">
      <i><xsl:apply-templates/></i>
   </xsl:template>
